...
sap.suite.ui.commons.TimelineItem.prototype.onBeforeRendering = function() {
	...
	var that = this;
	if (this._actionList){
		this._oldActionList = this._actionList;
	}
	
	this._actionList = this.getCustomAction();
	this._customActionChange = !this._isSameActionList(this._oldActionList, this._actionList);
	if (this._customActionChange){
		this._jamBar.removeAllContent();
	}
	if ((this._firstTime || this._customActionChange) && this.getParent() && this.getParent()._aFilterList && this.getParent().getEnableSocial() ) {
		this._jamBar.addContent(this._replyLink);
	}
	if ((this._firstTime || this._customActionChange) && this._actionList  && this._actionList.length > 0) {
		...
		for (var i = 0; i < this._actionList.length; i++) {
			...
			var actionLink = new sap.m.Link({
				text: this._actionList[i].getValue(),
				tooltip: this._actionList[i].getValue()
			});
