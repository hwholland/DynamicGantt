...
sap.ui.define([
	...
	SceneBuilder.prototype._setupView = function(position, zoom, pitch, yaw) {
		...
		position = new THREE.Vector3(parseFloat(tmp[0]), parseFloat(tmp[1]), parseFloat(tmp[2]));
		//move root instead of camera as implemented in VB
		var offset = new THREE.Vector3(-position.x, position.z, -position.y); //swapped components to compensate root rotation
		invert(offset);
		...
		rotX.makeRotationX(THREE.Math.degToRad(pitch + 180));

		var rotZ = new THREE.Matrix4();
		rotZ.makeRotationZ(THREE.Math.degToRad(-(yaw + 180)));
